...
void XBiotSavart :: SetMapMesh(const int mz, const int mr, const int mp)
{
  if (!fMapMesh)  fMapMesh = new int[3];
  ...
  if (fHC.size() != 0) {
    ...
    for (int i=0; i<fMapMesh[0]+1; i++) {
      ...
      for (int j=0; j<fMapMesh[1]+1; j++) {
        Z = fMapRange[1] + j*dz + dz/2.;
        // calculate A_phi
        A[i][j] = calpotential(R, Z);
...
double XBiotSavart :: calpotential(const double z, const double r) const
{
  const double dr = (fMapRange[1] - fMapRange[0]) / fMapMesh[0];
  const double dz = (fMapRange[3] - fMapRange[2]) / fMapMesh[1];
  const double dp = (2 * M_PI - 0.) / fMapMesh[2];

  double R, Z, PHI;
  double rPQ = 0.;
  double A   = 0.;

  for (int i=0; i<fMapMesh[0]; i++) {    // R
    R = fMapRange[0] + i*dr + dr/2.;

    for (int j=0; j<fMapMesh[2]; j++) {  // PHI
      PHI = 0. + j*dp + dp/2.;

      for (int k=0; k<fMapMesh[1]; k++) {  // Z
        Z = fMapRange[2] + k*dz + dz/2.;  
...
XFieldContainer* XBiotSavart :: GetField(const int iz, const int jr) const
{
